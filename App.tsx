...
const PendingApprovalView: React.FC<{ profile: any; onSignOut: () => void }> = ({
  ...
  return (
    <div className="h-screen w-screen flex flex-col items-center justify-center bg-white p-10 text-center animate-in fade-in duration-700">
      ...
      <div className="bg-gray-50 p-6 rounded-[2rem] w-full max-w-sm mb-12 flex items-center gap-6 border border-gray-100">
        ...
        <div className="text-left">
          ...
          <p className="text-lg font-black text-gray-900">
            {import.meta.env.VITE_SUPPORT_PHONE || '(00) 00000-0000'}
          </p>
        </div>
      </div>
      ...
      <button
        onClick={onSignOut}
        className="text-gray-500 font-black flex items-center gap-3 uppercase text-xs tracking-widest hover:bg-gray-100 px-8 py-4 rounded-2xl transition-all active:scale-95"
      >
        <LogOut size={20} /> Sair
      </button>
    </div>
...
const AppContent: React.FC = () => {
  ...
  if (!session && !isLoading) {
    return (
      <div className="bg-[#F8F9FC] min-h-screen">
        ...
        {showProfileModal && (
          <div
            className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-end md:items-center justify-center animate-in fade-in duration-300"
            onClick={() => setShowProfileModal(false)} // Fecha ao clicar fora
          >
            <div
              className="w-full h-[92dvh] md:h-[85vh] md:max-w-xl bg-white shadow-2xl rounded-t-[3rem] md:rounded-[3rem] overflow-hidden flex flex-col animate-in slide-in-from-bottom duration-500"
  ...
  // Tela de carregamento: só bloqueia se está carregando.
  // O perfil local vem do localStorage instantaneamente, então nunca ficamos sem perfil quando há sessão.
  if (isLoading) {
    return (
  ...
  return (
    <div className="bg-[#F8F9FC] min-h-screen">
      ...
      {showProfileModal && (
        ...
        >
          <div
            className="w-full h-[90vh] md:h-auto md:max-w-xl bg-white shadow-2xl rounded-t-[3rem] md:rounded-[3rem] overflow-hidden animate-in slide-in-from-bottom duration-500"
            onClick={e => e.stopPropagation()} // Impede fechar ao clicar dentro do modal
          >
            <AuthView onClose={() => setShowProfileModal(false)} />
          </div>
