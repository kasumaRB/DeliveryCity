...
export const AuthView: React.FC<AuthViewProps> = ({ onClose }) => {
  ...
  if (mode === 'SUPPORT') {
    if (!supportWhatsapp) {
      supabase.from('platform_settings').select('support_whatsapp').eq('id', 1).single()
    ...
    return (
      <div className="flex flex-col h-full bg-white animate-in slide-in-from-right duration-300">
        ...
        <div className="flex-1 p-8 space-y-6">
          {supportWhatsapp && (
            <a
              href={`https://wa.me/55${supportWhatsapp.replace(/\D/g, '')}?text=${encodeURIComponent('Olá! Preciso de ajuda com o DeliveryCity.')}`}
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center gap-4 w-full bg-green-500 hover:bg-green-600 text-white p-4 rounded-2xl font-bold text-sm active:scale-95 transition-all"
            >
              ...
              <div>
                <p className="font-black">Falar via WhatsApp</p>
                <p className="text-xs text-white/80 font-medium">Atendimento rápido com nossa equipe</p>
              </div>
            </a>
          )}
          <div className="bg-orange-50 p-6 rounded-3xl border border-orange-100">
            <p className="text-sm text-orange-800 font-medium">
              Ou envie uma mensagem abaixo e nossa equipe entrará em contato.
            </p>
          </div>
          <textarea
